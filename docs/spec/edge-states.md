...
표에 열이 없는 상태가 하나 더 있다 — **전용 화면을 두지 않은 실패**다. 위 다섯 사유 중 어느 것도 아닌 실패가 전부 여기로 오므로 실제로는 가장 자주 뜨는 실패 화면이고, 무엇을 띄울지는 실패 표가 정한다. §2.7 에 따로 적었다.
...
### 2.7 전용 화면을 두지 않은 실패 (Generic)

`FeedFailureReason.Generic` — 네트워크 단절도 점검도 아닌 나머지 전부다. 400·403·404·409·422·429·500 이 여기로 오고, 401 도 상태로는 여기 실리지만 §2.6 의 이동이 먼저 화면을 걷어낸다. **화면 한 장을 쓰는 자리는 `CareerCompassFailureState` 로 그린다**(#222) — 전에는 「검색 결과 없음」 부품을 돌려 써 삽화가 🔍 였고, 사용자는 서버가 500 을 준 것을 자기 조건이 잘못된 것으로 읽었다. 이제 삽화(⚠️)·문구·행동이 빈 결과와 갈리고, 버튼은 표의 행동이 있을 때만 붙는다.

**이 사유는 원 실패가 [실패 표](error-copy.md)의 어느 행이었는지를 들고 간다**(`Generic(kind)`, #342). 전에는 여기 오는 실패가 갈래까지 `Unexpected` 로 굳어, 삭제된 공고에 딥링크로 들어간 사용자가 「공고를 찾을 수 없어요」 대신 「문제가 생겼어요 / 잠시 후 다시 시도해 주세요」와 재시도 버튼을 봤다. 눌러도 같은 404 가 돌아오는 버튼이다. 아래 표의 문구는 그 행이 `Unexpected` 일 때의 것이고, 404·403·422·429 는 표의 제 행으로 나간다.

| 화면 | 컴포넌트 | 문구(리소스 = 값) | 버튼 → 동작 |
|---|---|---|---|
| 피드 홈 | `CareerCompassFailureState` | 실패 표의 `kind`×`Posting` 행(#204·#342). `Unexpected` = 공고를 불러오지 못했어요 / 잠시 후 다시 시도해 주세요 · 404 = 공고를 찾을 수 없어요 / 삭제됐거나 마감돼 내려간 공고예요 | 「다시 시도」(`core_ui_state_retry`) → `retry()` — 표의 `isRetryable` 이 거짓이면 버튼 없음 · 조건이 걸려 있으면 「조건 지우고 다시 보기」도 붙는다(§4 — 판정은 갈래가 아니라 사유가 한다) |
| 내 게시판 | `CareerCompassFailureState` | 실패 표의 `kind`×`Board` 행(#204·#342). `Unexpected` = 게시판을 불러오지 못했어요 / 잠시 후 다시 시도해 주세요 · 404 = 게시판을 찾을 수 없어요 / 이미 지워진 게시판이에요. 내 게시판 목록을 새로고침해 주세요 | 「다시 시도」(`core_ui_state_retry`) → `retryLoad()` — 표의 `isRetryable` 이 거짓이면 버튼 없음 |
...
> **#204 에서 정리됐다** — API_SPEC §9 의 에러 코드 14종이 [`docs/spec/error-copy.md`](error-copy.md) 의 표로 모였다. 코드마다 제목·본문과 「사용자가 할 수 있는 일」이 정해져 있고, **재시도해도 답이 갈리지 않는 실패에는 재시도가 붙지 않는다** — 상한 초과는 「정리하러 가기」로, 프로필 미완성은 「프로필 입력하기」로 갈린다. 화면이 `FailureSurface` 를 넘기면 표가 명사까지 채운다(공고 / 게시판). #342 전에는 그 표에 닿는 실패가 `Unexpected` 한 행뿐이라 나머지 열세 행이 피드에서는 도달 불가였다.
